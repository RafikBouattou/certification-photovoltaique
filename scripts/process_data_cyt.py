...
def process_and_merge_data():
    ...
    generation_df = pd.read_csv(r"C:\Users\Flipper\data-cleaner-frontend\temp_dataset\Dataset\Time series dataset\PV generation dataset\PV stations without panel level optimizer\Site level dataset\CYT (Station 1).csv")
    ...
    # Resample irradiance data to hourly intervals and calculate the mean
    irradiance_resampled_df = irradiance_df.resample('H').mean()

    # Merge the two dataframes
    merged_df = pd.merge(generation_df, irradiance_resampled_df, left_index=True, right_index=True, how='inner')

    # Save the merged dataframe to a new CSV file
    merged_df.to_csv(r"C:\Users\Flipper\CYT_Station_1_merged.csv")

    print("Data processing and merging complete. Merged file saved as CYT_Station_1_merged.csv")
